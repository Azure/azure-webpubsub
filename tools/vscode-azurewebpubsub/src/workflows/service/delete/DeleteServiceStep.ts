...
export class DeleteServiceStep extends AzureWizardExecuteStep<IPickServiceContext> {
    ...
    public async execute(context: IPickServiceContext, progress: Progress<{ message?: string | undefined; increment?: number | undefined }>): Promise<void> {
        if (!context.subscription || !context.serviceName || !context.resourceGroupName) {
            throw new Error(localize("deleteWebPubSubError", "Invalid IPickServiceContext, subscription = {0}, serviceName = {1}, resourceGroupName = {2}", context.subscription?.subscriptionId, context.serviceName, context.resourceGroupName));
        }

        const client = await createAzureApiClient([context, context.subscription]);
        progress.report({ message: localize('deletingWebPubSub', 'Deleting Web PubSub resource "{0}", please wait...', context.serviceName)});
        await client.webPubSub.beginDeleteAndWait(context.resourceGroupName, context.serviceName);
    }
