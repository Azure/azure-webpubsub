...
import { Dialog, DialogTrigger, DialogSurface, DialogTitle, DialogBody, DialogContent, Tab, TabList, CompoundButton, Button } from "@fluentui/react-components";
...
export const Playground = (props: PlaygroundProps) => {
  ...
  const [hubNames, setHubs] = useState<string[]>([]);
  ...
  useEffect(() => {
    console.log(`data.serviceConfiguration.hubNames changed!, hubNames=${data.serviceConfiguration.hubNames}`);
    setHubs(data.serviceConfiguration.hubNames ?? []);
  }, [data.serviceConfiguration.hubNames]);

  useEffect(() => {
    hubNames && setHub(hubNames[0]);
  }, [hubNames]);

  useEffect(() => {
    console.log(`hub or hubnames changed!, hub=${hub}, hubNames=${hubNames}`);
    const fetchUrl = async () => {
      const newUrl = await dataFetcher.invoke("getClientAccessUrl", { hub, userId: "", roles: [], groups: [] });
      setUrl(newUrl);
    };
    if (!hub) return;
    fetchUrl();
    ...
    <div className="d-flex flex-column flex-grow">
      <div>
        <div className="d-flex flex-row flex-grow">
          <div>
            <VSCodeTag>Hub</VSCodeTag>
            <VSCodeDropdown id="hub-name" onChange={(ev) => { setHub((ev.target as any).value); }} >
              {
                hubNames.map((hub) => <VSCodeOption key={hub} value={hub}>{hub}</VSCodeOption>)
              }
            </VSCodeDropdown>
          </div>
