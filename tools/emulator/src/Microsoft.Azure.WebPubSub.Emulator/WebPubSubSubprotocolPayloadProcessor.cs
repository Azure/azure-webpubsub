...
internal abstract class WebPubSubSubprotocolPayloadProcessor : IClientPayloadProcessor
{
    private readonly ConnectionManager _connections;
    private readonly IWebPubSubConnectionLifetimeHandler _lifetimeHandler;
    private readonly ILogger _logger;
    private readonly IWebPubSubClientDataProtocol _protocol;
    private readonly WebPubSubTokenService _tokenService;

    protected WebPubSubSubprotocolPayloadProcessor(
        ConnectionManager connections,
        IWebPubSubConnectionLifetimeHandler lifetimeHandler,
        IWebPubSubClientDataProtocol protocol,
    ...
    {
        // Preserve the existing JSON close descriptions while identifying other protocols.
        var protocolName = _protocol.Name == WebPubSubJsonV1PayloadProcessor.SubprotocolName
            ? "JSON"
            : _protocol.Name;
        if (messageType != _protocol.MessageType)
        {
            return PayloadProcessingResult.Close(
                WebSocketCloseStatus.InvalidMessageType,
                $"The {protocolName} subprotocol requires {_protocol.MessageType.ToString().ToLowerInvariant()} messages.");
        }
        ...
        catch (InvalidDataException exception)
        {
            _logger.LogWarning(
                exception,
                "Connection {ConnectionId} sent an invalid {ProtocolName} protocol message.",
                connection.ConnectionId,
                _protocol.Name);
            return PayloadProcessingResult.Close(
                WebSocketCloseStatus.InvalidPayloadData,
                $"The client message is not a valid {protocolName} protocol message.");
        }
