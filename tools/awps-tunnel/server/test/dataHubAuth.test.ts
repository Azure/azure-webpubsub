...
// Replace the sqlite-backed repo so the test does not need the native module.
// Persistence is not part of the security boundary under test.
jest.mock("../dataRepo", () => ({
...
// Minimal fake tunnel: DataHub only reads endpoint/hub/getLiveTraceUrl at construction
// and forwards token RPCs to it. None of these need a real Azure Web PubSub connection.
...
async function startHub(auth: DashboardAuthOptions): Promise<Harness> {
  ...
  // Constructing DataHub attaches the authenticated Socket.IO server to `server`.
...
// Attempt a connection and resolve with the outcome instead of throwing.
function tryConnect(
  port: number,
...
describe("DataHub dashboard authentication", () => {
  ...
  it("rejects a correct token from a disallowed origin", async () => {
    ...
    expect(res.connected).toBe(false);
    // allowRequest rejection surfaces as a transport/handshake error, not "unauthorized".
