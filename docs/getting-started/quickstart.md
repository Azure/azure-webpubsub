...
**Keys** tab contains the **Connection String** for the service, as well as a **Client URL Generator** for quick test purpose. You can connect to the service using URL generated by this generator, e.g. `new WebSocket({generated_url})`. In real-world applications, SDKs are provided to generate client URLs from **Connection String**.
...
- **Roles**: Specifies the roles for the connection. It can be used when the connection connects with `json.webpubsub.azure.v1` subprotocol that empowers the client to join/leave/send to groups. Here is a sample using the [subprotocol](./using-pubsub-subprotocol/js-work-with-subprotocols.md).
