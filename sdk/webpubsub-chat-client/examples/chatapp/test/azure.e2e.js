...
try {
  ...
  assert.equal(alice.userId, aliceId);
  assert.equal(bob.userId, bobId);
  ...
  const room = await alice.createRoom(`E2E ${suffix}`, [bob.userId]);
  assert.equal((await roomJoined).roomId, room.roomId);
