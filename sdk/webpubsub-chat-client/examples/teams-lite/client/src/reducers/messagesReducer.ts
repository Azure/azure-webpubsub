...
export type MessagesAction =
  ...
  | { type: "completeMessage"; payload: { messageId: string; content?: string; sender: string; isFromCurrentUser: boolean; isSystemMessage?: boolean } };
...
export function messagesReducer(state: MessagesState, action: MessagesAction): MessagesState {
  switch (action.type) {
    ...
    case "completeMessage": {
      const { messageId, content, sender, isFromCurrentUser, isSystemMessage } = action.payload;
      const existingIndex = state.findIndex((m) => m.id === messageId);
      if (existingIndex >= 0) {
        const next = [...state];
        next[existingIndex] = {
