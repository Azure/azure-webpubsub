...
export const AvatarWithOnlineStatus: React.FC<AvatarWithOnlineStatusProps> = ({
  ...
  // When ephemeral messages are disabled, all users appear online by default
  const isOnline = shouldShowOnlineStatus && (
    !chatContext?.ephemeralMessagesEnabled || chatContext?.onlineStatus[userOrRoomId]?.isOnline || false
  );
