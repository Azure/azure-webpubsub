...
import { DEFAULT_ROOM_ID, GLOBAL_METADATA_ROOM_ID } from "../lib/constants";
import { LoginDialog } from "../components/LoginDialog";

// Feature flag for ephemeral messages (ping, typing indicators)
// When disabled, all users will appear as online by default
const ENABLE_EPHEMERAL_MESSAGES = true;

// Online status configuration
const PING_INTERVAL_MS = 2500; // Send ping every X seconds
const OFFLINE_TIMEOUT_MS = 2 * PING_INTERVAL_MS; // Mark as offline if no ping received within 20 seconds

// Typing status configuration
const TYPING_TIMEOUT_MS = 3000; // Mark as not typing if no typing indicator received within 3 seconds
...
export const ChatClientProvider: React.FC<ChatClientProviderProps> = ({ children }) => {
  ...
  const [roomMembersUpdateTrigger, setRoomMembersUpdateTrigger] = React.useState(0);
  ...
  const [successNotification, setSuccessNotificationInternal] = React.useState<string>("");
  
  // Wrapper to track all calls
  const setSuccessNotification = React.useCallback((value: string) => {
    console.log('[Provider] setSuccessNotification called with:', value, 'stack:', new Error().stack?.split('\n').slice(1, 4).join(' <- '));
    setSuccessNotificationInternal(value);
  }, []);
  ...
  const updateRoomMessages = React.useCallback((targetRoomId: string | undefined, action: MessagesAction) => {
    ...
    const isSystemMessage = action.type === "completeMessage" && action.payload?.isSystemMessage;
    if (!isCurrentRoom && !isSystemMessage && (action.type === "completeMessage" || action.type === "streamEnd")) {
    ...
    const initializeClient = async () => {
      ...
      try {
        ...
        chatClient.on("room-joined", (event) => {
          ...
          // Check if room already exists (means we created it ourselves)
          const existingRoom = roomsRef.current.find(r => r.roomId === room.roomId);
          
          if (!existingRoom) {
            // Only add to list if not already present (we were added by someone else)
            ...
            // Only show notification and system message if we were added by someone else
            // (not if we created the room ourselves)
            const isPrivateChat = room.roomId.startsWith('private-');
            let notificationText: string;
            let systemMessageText: string;
            
            if (isPrivateChat) {
              // Extract other user from private room ID (format: private-user1-user2)
              const parts = room.roomId.split('-');
              const otherUser = parts[1] === chatClient.userId ? parts[2] : parts[1];
              notificationText = `You started a private chat with ${otherUser}`;
              systemMessageText = `Private chat with ${otherUser}`;
            } else {
              notificationText = `You have been added to room: ${room.title}`;
              systemMessageText = `You joined "${room.title}"`;
            }
            
            // Add a system message to trigger room sorting (new room goes to top)
            updateRoomMessages(room.roomId, { 
              type: "completeMessage", 
              payload: { 
                messageId: `system-joined-${room.roomId}-${Date.now()}`,
            ...
            setUiNoticeRef.current({ type: "info", text: `🎉 ${notificationText}` });
            setSuccessNotification(notificationText);
          }
        ...
        chatClient.on("member-joined", (event) => {
          ...
          setSuccessNotification(`User ${userId} has joined room: ${title}`);
          
          // Trigger room members refresh (not messages)
        ...
        chatClient.on("member-left", (event) => {
          ...
          if (roomId === GLOBAL_METADATA_ROOM_ID) return;
          // Show success notification banner
          setSuccessNotification(`User ${userId} has left room: ${title}`);
        ...
        chatClient.on("room-left", (event) => {
          ...
          // If currently viewing this room, switch to default room
          if (roomIdRef.current === roomId) {
            setRoomIdRef.current(DEFAULT_ROOM_ID);
          }
          
          // Show UI notification
          setUiNoticeRef.current({ type: "info", text: `🚪 You have been removed from room: ${title}` });
        ...
        // Rooms are already created by server, just use chatClient.rooms from login
        ...
        // Send initial ping immediately to announce user is online (only if ephemeral messages are enabled)
        if (ENABLE_EPHEMERAL_MESSAGES) {
        ...
        // Start online status check interval (every 5 seconds) - only if ephemeral messages are enabled
        if (ENABLE_EPHEMERAL_MESSAGES) {
  ...
  // For preview display, skip system messages; for sorting, include all messages
  const getLastMessageForRoom = React.useCallback((roomId: string, includeSystemMessages: boolean = false): ChatMessage | null => {
    const rs = roomStatesRef.current.get(roomId);
    if (!rs || rs.messages.length === 0) {
      return null;
    }
    
    // If including system messages, just return the last one
    if (includeSystemMessages) {
      return rs.messages[rs.messages.length - 1];
    }
    
    // Otherwise, find the last non-system message for preview
    for (let i = rs.messages.length - 1; i >= 0; i--) {
      if (!rs.messages[i].isSystemMessage) {
  ...
  // Send typing indicator to a specific room (only if ephemeral messages are enabled)
  const sendTypingIndicator = React.useCallback((targetRoomId: string) => {
    if (!ENABLE_EPHEMERAL_MESSAGES) return; // Skip if ephemeral messages are disabled
  ...
  const value = React.useMemo(
    () => ({
      ...
      ephemeralMessagesEnabled: ENABLE_EPHEMERAL_MESSAGES,
    }),
    [client, connectionStatus, messages, isStreaming, sendMessage, clearMessages, uiNotice, unreadCounts, getLastMessageForRoom, roomMessagesUpdateTrigger, roomMembersUpdateTrigger, onlineStatus, typingStatus, sendTypingIndicator, getTypingUsersForRoom, successNotification],
  );
  return (
    <>
      <LoginDialog
