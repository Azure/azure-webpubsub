...
// Auction list
function renderAuctionList(flashRoomId) {
  ...
  for (const [roomId, auction] of sorted) {
    ...
    `;
    if (flashRoomId === roomId) {
      div.classList.add('bid-flash');
    }
...
async function placeBid(amount) {
  ...
  try {
    ...
    renderAuctionList(currentAuctionId);
  } catch (e) {
    const el = $('bid-error');
...
function renderParticipants(auction) {
  const container = $('participants');
  container.innerHTML = '';
  for (const userId of auction.members) {
    const span = document.createElement('span');
    span.className = 'px-2 py-1 rounded-full text-xs font-medium ' +
      (userId === client.userId ? 'bg-blue-100 text-blue-700 border border-blue-300' : 'bg-gray-100 text-gray-600 border border-gray-300');
