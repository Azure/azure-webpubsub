...
| `stopped` | `OnStoppedArgs` | The client transitioned to not-started (explicit `stop()` or transport-driven); `previousState` snapshots its `userId` and `rooms` before it stopped. |
