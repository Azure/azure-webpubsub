...
half3 vertexColor;
    ...
    void Surf(v2f IN, inout AvatarComponentSurfaceOutput o) {
        ...
        // required for the Dissolve effect
        o.worldPos = IN.worldPos;

        // required for Ghost effect 
    ...
    half4 LightingAvatarComponent(AvatarComponentSurfaceOutput s, half3 viewDir, AvatarShaderGlobalIllumination gi) {
        ...
        #if defined(ENABLE_CUSTOM_EFFECT)
            c = ApplyDissolveEffect(c, s.worldPos, normal);
        #endif

        #if defined(ENABLE_GHOST_EFFECT)
            c = ApplyGhostEffect(c, normal, s.worldPos, s.screenPos, s.uv);
        #endif
...
void Interpolate(float2 texcoord, float4 vertex, inout v2f o) {
    ...
    // screenPos required for Ghost effect
    v2f a;
    a.pos = UnityObjectToClipPos(vertex);
    o.screenPos = ComputeScreenPos(a.pos);
}
    ...
    void Interpolate(appdata v, OvrVertexData vertexData, inout v2f o) {
        // Call specific interpolation function for this shader
        Interpolate(v.uv, vertexData.position, o);
    }
#endif
