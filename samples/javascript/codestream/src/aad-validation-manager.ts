...
import AzureActiveDirectoryExpressMiddlewareBuilder from './aad-express-middleware'
...
export default class AzureActiveDirectoryValidationManager {
    ...
    constructor(options: AzureActiveDirectoryOptions) {
        ...
        setInterval(() => this.updateCertificates(options.tenantId), 10 * 60 * 1000)
    ...
    updateCertificates(tenantId: string) {
        ...
        self.loadOpenIdConfig(tenantId, function (result: any, err: Error) {
            ...
            self.loadSigningKeys(result.jwks_uri, function(certificates: string[], err: Error) {
                // TODO handle err
