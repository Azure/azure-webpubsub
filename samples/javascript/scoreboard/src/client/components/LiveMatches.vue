        ...
        <el-col :xs="24" :sm="24" :md="8" :lg="8" :xl="8" v-for="(c, i) in cards" :key="i">
            <el-card class="box-card">
                ...
                <el-row justify="space-around" class="horizontal-center">
                    <img class="thumbnail-image" :src="c.thumbnails[0].value" :alt="`team logo for ${c.teams[0]}`" :aria-label="`team logo for ${c.teams[0]}`"/>
                    {{ c.scores[0] + ' : ' + c.scores[1] }}
                    <img class="thumbnail-image" :src="c.thumbnails[1].value" :alt="`team logo for ${c.teams[1]}`" :aria-label="`team logo for ${c.teams[1]}`"/>
                </el-row>
            </el-card>
        </el-col>
    </el-row>
