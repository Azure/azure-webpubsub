...
async function initialize() {
  ...
  const socket = io(negotiateJson.endpoint, {
    ...
    query: {
      access_token: negotiateJson.token
    }
  });
  ...
  socket.on('updateBackground', data => {
    let i = data;
  ...
  diagram.onShapeUpdate((i, m) => socket.emit('updateShape', [author, i, m]));
  diagram.onShapePatch((i, d) => socket.emit('patchShape', [author, i, d]));
  diagram.onShapeAdd((i, m) => socket.emit('addShape', [i, m]));
  diagram.onShapeRemove(i => socket.emit('removeShape', [author, i]));
  diagram.onClear(() => socket.emit('clear', author));
  ...
  let app = new Vue({
    ...
    methods: {
      ...
      sendMessage: function () {
        ...
        socket.emit('newMessage', [author, this.name, this.draft]);
        this.draft = '';
      },
